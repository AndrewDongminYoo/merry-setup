...
45. The managed default cache must be keyed by the selected SDK family and resolved exact SDK version; Flutter must use the resolved Flutter version rather than its bundled Dart version. Projects selecting the same key deliberately share one mutable global activation namespace in which the last successful activation plan determines current tool versions. Project-specific or activation-plan-specific cache hashing, automatic cache cleanup, and concurrent activation guarantees are out of scope for v1. [L14]
