...
64. `bashrc` mode must idempotently replace one `merry-setup`-managed block in `~/.bashrc`, or append the block when absent. The block must safely quote and export the resolved `PUB_CACHE` and must set PATH with the exact SDK bin directory, `${PUB_CACHE}/bin`, the local tool bin directory, the npm global bin directory only when a selected bundle installed an executable there, and the pre-existing PATH without accumulating stale exact-version entries. [L14] [amended by Spec 0002]
65. `github` mode must write the resolved `PUB_CACHE` to `$GITHUB_ENV` and append the exact SDK bin directory, `${PUB_CACHE}/bin`, the local tool bin directory, and the npm global bin directory only when a selected bundle installed an executable there to `$GITHUB_PATH`; it must fail clearly before those writes when either environment file is unavailable. [L14] [amended by Spec 0002]
...
78. The Action must expose these string-valued public inputs: required `sdk`, required `bootstrap`, and optional `sdk-version`, `merry`, `merry-version`, `dart-packages`, `bundles`, `firebase-tools-version`, `precache`, `project-dir`, `trunk-path`, and `cache`. `merry` must accept only `true` or `false` and default to `true`. `cache` must accept only `true` or `false` and default to `false`. [L1] [L7] [L9] [L14] [amended by Spec 0002]
...
82. The Action must always execute full `setup`, must select `github` PATH persistence, must use `$GITHUB_PATH` or `$GITHUB_ENV` for values needed by later workflow steps, and must not expose a command selector or public outputs. When `cache` is `true`, the Action may run the read-only internal `resolve` command before full setup as Spec 0002 defines. [L9] [amended by Spec 0002]
83. Before invoking `trunk-io/trunk-action/setup`, `action/preflight.sh` must validate the cache and bootstrap enums and, unless bootstrap is `none`, the existence of a regular `pubspec.yaml` under the resolved project directory. It must not write files, install tools, or validate unrelated inputs. [L11] [amended by Spec 0002]
...
16. Action adapter tests must verify that `bootstrap=none` skips the manifest check, supported non-`none` values require `pubspec.yaml`, invalid bootstrap or cache values fail before Trunk setup, relative and absolute project directories including spaces resolve correctly, and no preflight failure reaches the delegated Trunk step. [L11] [amended by Spec 0002]
17. Action adapter tests must verify safe LF and CRLF multiline-input conversion, blank-line handling, GitHub `PUB_CACHE` and PATH persistence, upstream `TRUNK_PATH` consumption, the opt-in resolve and archive transport arguments, the unchanged `cache=false` setup arguments, and failure when required GitHub environment files are missing. [L9] [L14] [amended by Spec 0002]
...
- Action inputs for architecture selection, automatic SDK or bootstrap detection, per-tool booleans, generic npm packages, or command selection. Archive caching is available only through the `cache` input that Spec 0002 defines. [L9]
...
- Action or SDK caching beyond the checksum-bound upstream SDK archive cache that Spec 0002 defines.
