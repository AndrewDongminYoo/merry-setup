...
3. Cache-key tests must show that two runs differing only in resolved SDK version, precache plan, or runner architecture produce different SDK keys, that two runs differing only in activation plan or project lock files produce the same SDK key and different pub keys, that two precache plans naming the same targets in a different order or with repeats produce the same key, and that `stable` and the version it resolves to produce the same key.
